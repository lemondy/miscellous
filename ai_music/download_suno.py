...
def download_media(urls, music_directory, title_array):
    ...
    return saved_file_paths, total_duration  # Return the list of saved file paths
...
def process_urls(urls):
    ...
    for url in urls.splitlines():  # Split input by new lines for multiple URLs
        ...
        saved_file_paths, total_duration = download_media(media_urls, 'music', title_array)
    ...
    print("total duration sum:", total_duration_sum)
...
def rename_mp3(dataframe):
    ...
    for index, row in dataframe.iterrows():
        ...
        title = sanitize_filename(title)  # 清理文件名
        ...
        print("directory:", directory, "new_filename:", new_filename)
        
        # Rename the file
        try:
            os.rename(mp3_path, new_filename)
            results.append({"Old Path": mp3_path, "New Path": new_filename})
        except Exception as e:
            print(f"Error renaming file {mp3_path} to {new_filename}: {e}")
    
    return results  # Return the results of the renaming operation


def update_line_count(text):
    line_count = len(text.splitlines())  # 计算行数
    return f"行数: {line_count}"  # 返回行数信息
...
if __name__ == "__main__":
    with gr.Blocks() as iface:
        ...
        line_count_label = gr.Label(value="已输入的行数: 0")
        ...
        # 将更新行数的函数与文本框关联
        url_input.change(fn=update_line_count, inputs=url_input, outputs=line_count_label)
